-- ============================================================================
-- get_spending_ranking() calculava "mês atual"/"ano atual" com date_trunc no
-- fuso da sessão do Postgres (UTC por padrão) — uma retirada feita às 23h de
-- Brasília (02h UTC do dia seguinte) podia cair no mês/ano errado. Convertendo
-- explicitamente pra America/Sao_Paulo antes de truncar.
-- ============================================================================
...
begin
  ...
  return query
    ...
    left join withdrawals w on w.user_id = p.id
      ...
      and (
        p_period = 'all'
        or (p_period = 'month' and w.created_at >= (date_trunc('month', now() at time zone 'America/Sao_Paulo') at time zone 'America/Sao_Paulo'))
